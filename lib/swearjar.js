...
var swearjar = {
  ...
  checkForProfanity(text, start = 0, partialLength = 3) {
    ...
    if (this.profane(partialText)) {
      ...
      if(partialLength !== text.length) {
        if (start !== text.length - partialLength) {
          return this.checkForProfanity(text, ++start, partialLength);
        } else {
          return this.checkForProfanity(text, 0, ++partialLength)
        }
      }
    }
    return false;
